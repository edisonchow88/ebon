<!-- START: Modal -->
<div class="modal fade" id="modal-add-contact" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal">&times;</button>
            <h4 class="modal-title">Add Poi Contact</h4>
            </div>
        <div class="modal-body">
        	<div id="modal-add-contact-form-alert"></div>
            <div id="modal-add-contact-form-demo" class="text-center"></div>
            <!-- START: Nav tabs -->
            <?php
                $tab = 0;
            ...
            <form id="modal-add-contact-form">
                <input 
                    type="hidden" 
                    name="action"
                    value="add" 
                        ...
                        foreach($modal_input as $i) {
                            if(isset($i['tab'])) {
                                ...
                                if($i['type'] == 'textarea') {
                                    ...
                                    echo 'rows="'.$r.'" ';
                                    echo 'id="modal-add-contact-form-input-'.$i['id'].'" ';
                                    echo 'name="'.$i['name'].'" ';
                                    ...
                                    echo '<span id="modal-add-contact-form-text-'.$i['id'].'">';
                                    if($i['type'] == 'hidden') {
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-danger" onclick="addContact();">Save</button>
            </div>
        </div>
    </div>
</div>
...
<script>
	function addContact() {
		var form_element = document.querySelector("#modal-add-contact-form");
		...
		xmlhttp.onreadystatechange = function() {
			document.getElementById('modal-add-contact-form-alert').innerHTML = "";
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				...
				if(typeof json.warning != 'undefined') {
					...
					content += "</ul></div>";
					document.getElementById('modal-add-contact-form-alert').innerHTML = content;
				}
				...
				document.getElementById('modal-add-contact-form-alert').innerHTML = xmlhttp.responseText;
			}
		};
	...
	/*
	function updateAddContactDemo() {
		var name = document.getElementById('modal-add-contact-form-input-name').value;
		document.getElementById('modal-add-contact-form-demo').innerHTML = "<span style='text-transform:capitalize;'>"+name+"</span>";
	}
	
	$("#modal-add-contact-form").change(function(e) {
		updateAddContactDemo();
	});
