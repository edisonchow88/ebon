<!-- START: Modal -->
<div class="modal fade" id="modal-add-data" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-body">
        	<div id="modal-add-data-form-alert"></div>
            <div id="modal-add-data-form-demo" class="text-center"></div>
            <!-- START: Nav tabs -->
            <?php
                $tab = 0;
                        ...
                        foreach($modal_input as $i) {
                            if(isset($i['tab'])) {
                                ...
                                if($i['type'] == 'textarea') {
                                    ...
                                    echo 'rows="'.$r.'" ';
                                    echo 'id="modal-add-data-form-input-'.$i['id'].'" ';
                                    echo 'name="'.$i['name'].'" ';
                                    ...
                                    echo '<span id="modal-add-data-form-text-'.$i['id'].'">';
                                    if($i['type'] == 'hidden') {
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-danger" onclick="addData();">Save</button>
            </div>
        </div>
    </div>
</div>
...
<script>
	function addData() {
		var form_element = document.querySelector("#modal-add-data-form");
		...
		xmlhttp.onreadystatechange = function() {
			document.getElementById('modal-add-data-form-alert').innerHTML = "";
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				...
				if(typeof json.warning != 'undefined') {
					...
					content += "</ul></div>";
					document.getElementById('modal-add-data-form-alert').innerHTML = content;
					$('#modal-add-data').scrollTop(0);
				}
				...
				document.getElementById('modal-add-data-form-alert').innerHTML = xmlhttp.responseText;
			}
		};
	...
	function updateAddDataDemo() {
		var name = document.getElementById('modal-add-data-form-input-name').value;
		document.getElementById('modal-add-data-form-demo').innerHTML = "<span style='text-transform:capitalize;'>"+name+"</span>";
	}
	
	$("#modal-add-data-form").change(function(e) {
		updateAddDataDemo();
	});
