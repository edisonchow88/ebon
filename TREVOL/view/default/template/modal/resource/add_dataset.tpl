<!-- START: Modal -->
<div class="modal fade" id="modal-add-dataset" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-body">
        	<div id="modal-add-dataset-form-alert"></div>
            <div id="modal-add-dataset-form-demo" class="text-center"></div>
            <!-- START: Nav tabs -->
            <?php
                $tab = 0;
            ...
            <form id="modal-add-dataset-form">
                <input 
                    type="hidden" 
                    name="action"
                    value="add" 
                        ...
                        foreach($modal_input as $i) {
                            if(isset($i['tab'])) {
                                ...
                                if($i['type'] == 'textarea') {
                                    ...
                                    echo 'rows="'.$r.'" ';
                                    echo 'id="modal-add-dataset-form-input-'.$i['id'].'" ';
                                    echo 'name="'.$i['name'].'" ';
                                    ...
                                    echo '<span id="modal-add-dataset-form-text-'.$i['id'].'">';
                                    if($i['type'] == 'hidden') {
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-danger" onclick="addDataset();">Save</button>
            </div>
        </div>
    </div>
</div>
...
<script>
	function addDataset() {
		var form_element = document.querySelector("#modal-add-dataset-form");
		...
		xmlhttp.onreadystatechange = function() {
			document.getElementById('modal-add-dataset-form-alert').innerHTML = "";
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				...
				if(typeof json.warning != 'undefined') {
					...
					content += "</ul></div>";
					document.getElementById('modal-add-dataset-form-alert').innerHTML = content;
					$('#modal-add-dataset').scrollTop(0);
				}
				...
				document.getElementById('modal-add-dataset-form-alert').innerHTML = xmlhttp.responseText;
			}
		};
	...
	function updateAddDatasetDemo() {
		var name = document.getElementById('modal-add-dataset-form-input-name').value;
		document.getElementById('modal-add-dataset-form-demo').innerHTML = "<span style='text-transform:capitalize;'>"+name+"</span>";
	}
	
	$("#modal-add-dataset-form").change(function(e) {
		updateAddDatasetDemo();
	});
