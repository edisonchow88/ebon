<!-- START: Modal -->
<div class="modal fade" id="modal-add-status" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-body">
        	<div id="modal-form-add-status-alert"></div>
            <div id="modal-form-add-status-demo" class="text-center"></div>
            <form id="modal-form-add-status">
                <input 
                    type="hidden" 
                    name="action"
                    value="add" 
                    	...
                    	if(isset($i['section'])) {
                            ...
                            echo '<div class="input-group col-sm-8 col-xs-12">';
                              if($i['type'] == 'textarea') {
                                ...
                                echo 'rows="5" ';
                                echo 'id="modal-form-add-status-input-'.$i['id'].'" ';
                                echo 'name="'.$i['name'].'" ';
                                ...
                                echo '<span id="modal-form-add-status-text-'.$i['id'].'">';
                                if($i['type'] == 'hidden') {
                                    echo $i['text'];
                                }
                                echo '</span>';
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-danger" onclick="addStatus();">Save</button>
            </div>
        </div>
    </div>
</div>
...
<script>
	function addStatus() {
		var form_element = document.querySelector("#modal-form-add-status");
		...
		xmlhttp.onreadystatechange = function() {
			document.getElementById('modal-form-add-status-alert').innerHTML = "";
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				...
				if(typeof json.warning != 'undefined') {
					...
					content += "</ul></div>";
					document.getElementById('modal-form-add-status-alert').innerHTML = content;
				}
