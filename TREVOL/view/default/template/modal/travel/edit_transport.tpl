<!-- START: Modal -->
<div class="modal fade" id="modal-edit-transport" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-body">
            ...
            <form id="modal-form-get-transport">
                <input 
                    type="hidden" 
                    name="action"
                    value="get" 
                ...
                <input
                    type="hidden"
                    id="modal-form-get-transport-input-transport-id" 
                    name="transport_id" 
                />
            </form>
        </div>
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-danger" onclick="editTransport();">Save</button>
            </div>
        </div>
    </div>
</div>
...
<script>
	function editTransport() {
		var form_element = document.querySelector("#modal-form-edit-transport");
		...
		xmlhttp.onreadystatechange = function() {
			document.getElementById('modal-form-edit-transport-alert').innerHTML = "";
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				...
				if(typeof json.warning != 'undefined') {
					...
					content += "</ul></div>";
					document.getElementById('modal-form-edit-transport-alert').innerHTML = content;
				}
	...
	function getTransport() {
		var form_element = document.querySelector("#modal-form-get-transport");
		var form_data = new FormData(form_element);
		var xmlhttp = new XMLHttpRequest();
		var url = "<?php echo $modal_ajax['travel/ajax_transport']; ?>";
		var data = "";
		...
		xmlhttp.onreadystatechange = function() {
			...
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				...
				<?php
                    foreach($modal_input as $i) {
						if(!isset($i['section'])) { 
							echo "document.getElementById('modal-form-edit-transport-input-".$i['id']."').value = json.".$i['name'].";";
							if(isset($i['json'])) {
								echo "document.getElementById('modal-form-edit-transport-text-".$i['id']."').innerHTML = json.".$i['json'].";";
							}
						}
					}
				?>
