<!-- START: Modal -->
<div class="modal fade" id="modal-edit-trip" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-body">
            ...
            <form id="modal-form-get-trip">
                ...
                <input
                    type="hidden"
                    id="modal-form-get-trip-input-trip-id" 
                    name="trip_id" 
                />
            </form>
        </div>
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-danger" onclick="editTrip();">Save</button>
            </div>
        </div>
    </div>
</div>
...
<script>
	function editTrip() {
		var form_element = document.querySelector("#modal-form-edit-trip");
		var form_data = new FormData(form_element);
		var xmlhttp = new XMLHttpRequest();
		var url = "<?php echo $modal_ajax['travel/ajax_trip']; ?>";
		var data = "";
		var query = url + data;
		xmlhttp.onreadystatechange = function() {
			document.getElementById('modal-form-edit-trip-alert').innerHTML = "";
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				...
				if(typeof json.warning != 'undefined') {
					...
					content += "</ul></div>";
					document.getElementById('modal-form-edit-trip-alert').innerHTML = content;
				}
	...
	function getTrip() {
		var form_element = document.querySelector("#modal-form-get-trip");
		var form_data = new FormData(form_element);
		var xmlhttp = new XMLHttpRequest();
		var url = "<?php echo $modal_ajax['travel/ajax_trip']; ?>";
		var data = "";
		...
		xmlhttp.onreadystatechange = function() {
			...
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				...
				<?php
                    foreach($modal_input as $i) {
						if(!isset($i['section'])) { 
							echo "document.getElementById('modal-form-edit-trip-input-".$i['id']."').value = json.".$i['name'].";";
							if(isset($i['json'])) {
								echo "document.getElementById('modal-form-edit-trip-text-".$i['id']."').innerHTML = json.".$i['json'].";";
							}
						}
					}
				?>
				...
				<!-- if connection failed -->
				document.getElementById('modal-form-edit-trip-alert').innerHTML = xmlhttp.responseText;
			}
		};
		xmlhttp.open("POST", query, true);
		xmlhttp.send(form_data);
