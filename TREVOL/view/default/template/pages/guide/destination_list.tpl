...
<div id="content" class="panel panel-default">
	<div class="panel-heading col-xs-12">
        ...
        <div class="col-xs-2 text-right"><a href="<?php echo $link['guide/destination_form'];?>" class="btn btn-danger" role="button">Add Destination</a></div>
	</div>
    	...
    	<table id="grid" class="table table-condensed table-hover table-striped">
            <thead>
                <tr>
                    ...
                    <th data-column-id="descendant" data-formatter="descendant" data-sortable="false">Descendant</th>
    ...
    <div class="panel-footer col-xs-12" style="padding:10px;">
    	<div class="alert alert-warning">
            ...
            <ul>
            	<li>Function to show Similar Destination</li>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-body">
            <div class="alert alert-danger" role="alert">
                Are you sure you want to delete <b>Destination Type #<span id="destination-id-delete-text"></span></b> ?
            </div>
            <form id="request-delete" action="<?php echo $link['guide/destination_post'];?>" method="post">
                <input 
                    type="hidden" 
                    id="destination-id-delete-input" 
                    name="destination_id" 
                />
                <input 
                    type="hidden" 
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-danger" onclick="deleteDestination();">Delete</button>
            </div>
        </div>
    </div>
</div>
...
<script>
	var grid = $("#grid").bootgrid({
		...
		formatters: {
			...
			"descendant": function(column, row)
			{
				if(row.descendant > 0) {
					return "<div class=\"text-center\"><a class=\"btn btn-default search-descendant\" data-row-name=\"" + row.name + "\">" + row.descendant + "</a></div>";
				}
				else {
					return "<div class=\"text-center\">0</div>";
				}
	...
	}).on("loaded.rs.jquery.bootgrid", function()
	{
		...
		{
			window.location.href = "<?php echo $link['guide/destination_form']; ?>\&destination_id=" + $(this).data("row-id");
		})
		.end().find(".command-delete").on("click", function(e)
		{
			document.getElementById("destination-id-delete-text").innerHTML = $(this).data("row-id");
			document.getElementById("destination-id-delete-input").value = $(this).data("row-id");
