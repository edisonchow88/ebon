...
<div id="content" class="panel panel-default">
	<div class="panel-heading col-xs-12">
    	<div class="col-xs-3 text-left"><a href="<?php echo $link['guide/destination']; ?>" class="btn btn-default" role="button">Back</a></div>
    	<div class="col-xs-6 text-center"><h5>Destination Description</h5></div>
        <div class="col-xs-3 text-right"><a data-toggle="modal" data-target="#modal-add-description" class="btn btn-danger" role="button">Add Destination Description</a></div>
	</div>
...
<!-- START: Modal -->
	<?php echo $modal_add_description; ?>
    <?php echo $modal_edit_description; ?>
	<?php echo $modal_delete_description; ?>
<!-- END: Modal -->
...
<script>
	var grid = $("#grid").bootgrid({
		...
		formatters: {
			...
			{
				return "<button type=\"button\" class=\"btn btn-default command-edit\" data-toggle=\"modal\" data-target=\"#modal-edit-description\" data-row-id=\"" + row.id + "\"><span class=\"fa fa-fw fa-pencil\"></span></button> " + 
					"<button type=\"button\" class=\"btn btn-default command-delete\" data-toggle=\"modal\" data-target=\"#modal-delete-description\" data-row-id=\"" + row.id + "\"><span class=\"fa fa-fw fa-trash-o\"></span></button>";
	...
	}).on("loaded.rs.jquery.bootgrid", function()
	{
		...
		{
			document.getElementById("modal-get-description-form-input-description-id").value = $(this).data("row-id");
			getDescription(); <!-- the code is written in modal tpl -->
			$($(this).attr("data-target")).modal("show");
		})
		.end().find(".command-delete").on("click", function(e)
		{
			document.getElementById("modal-delete-description-form-input-description-id").value = $(this).data("row-id");
			document.getElementById("modal-delete-description-form-text-description-id").innerHTML = $(this).data("row-id");
			$($(this).attr("data-target")).modal("show");
		});
	});
