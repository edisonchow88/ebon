...
<div id="content" class="panel panel-default">
	<div class="panel-heading col-xs-12">
    	<div class="col-xs-2 text-left"><a href="<?php echo $link['guide/destination']; ?>" class="btn btn-default" role="button">Back</a></div>
    	<div class="col-xs-8 text-center"><h5>Destination Google</h5></div>
        <div class="col-xs-2 text-right"><a data-toggle="modal" data-target="#modal-add-google" class="btn btn-danger" role="button">Add Destination Google</a></div>
	</div>
...
<!-- START: Modal -->
	<?php echo $modal_add_google; ?>
    <?php echo $modal_edit_google; ?>
	<?php echo $modal_delete_google; ?>
<!-- END: Modal -->
...
<script>
	var grid = $("#grid").bootgrid({
		...
		formatters: {
			...
			"g_url": function(column, row)
			{
				return "<a href='"+row.g_url+"' target='_blank'>Link</a>";
			},
			"commands": function(column, row)
			{
				return "<button type=\"button\" class=\"btn btn-default command-edit\" data-toggle=\"modal\" data-target=\"#modal-edit-google\" data-row-id=\"" + row.id + "\"><span class=\"fa fa-fw fa-pencil\"></span></button> " + 
					"<button type=\"button\" class=\"btn btn-default command-delete\" data-toggle=\"modal\" data-target=\"#modal-delete-google\" data-row-id=\"" + row.id + "\"><span class=\"fa fa-fw fa-trash-o\"></span></button>";
	...
	}).on("loaded.rs.jquery.bootgrid", function()
	{
		...
		{
			document.getElementById("modal-get-google-form-input-google-id").value = $(this).data("row-id");
			getGoogle(); <!-- the code is written in modal tpl -->
			$($(this).attr("data-target")).modal("show");
		})
		.end().find(".command-delete").on("click", function(e)
		{
			document.getElementById("modal-delete-google-form-input-google-id").value = $(this).data("row-id");
			document.getElementById("modal-delete-google-form-text-google-id").innerHTML = $(this).data("row-id");
			$($(this).attr("data-target")).modal("show");
		});
	});
