...
<div id="content" class="panel panel-default">
	<div class="panel-heading col-xs-12">
    	...
    	<div class="col-xs-8 text-center"><h5>Destination Image</h5></div>
        <div class="col-xs-2 text-right"><a data-toggle="modal" data-target="#modal-add-image" class="btn btn-danger" role="button">Add Destination Image</a></div>
	</div>
...
<!-- START: Modal -->
    ...
    <?php echo $modal_edit_image; ?>
	<?php echo $modal_delete_image; ?>
<!-- END: Modal -->
...
<script>
	var grid = $("#grid").bootgrid({
		...
		formatters: {
			...
			{
				return "<button type=\"button\" class=\"btn btn-default command-edit\" data-toggle=\"modal\" data-target=\"#modal-edit-image\" data-row-id=\"" + row.id + "\"><span class=\"fa fa-fw fa-pencil\"></span></button> " + 
					"<button type=\"button\" class=\"btn btn-default command-delete\" data-toggle=\"modal\" data-target=\"#modal-delete-image\" data-row-id=\"" + row.id + "\"><span class=\"fa fa-fw fa-trash-o\"></span></button>";
	...
	}).on("loaded.rs.jquery.bootgrid", function()
	{
		...
		{
			document.getElementById("modal-get-image-form-input-relation-id").value = $(this).data("row-id");
			getImage(); <!-- the code is written in modal tpl -->
			$($(this).attr("data-target")).modal("show");
		})
		.end().find(".command-delete").on("click", function(e)
		{
			document.getElementById("modal-delete-image-form-input-relation-id").value = $(this).data("row-id");
			document.getElementById("modal-delete-image-form-text-relation-id").innerHTML = $(this).data("row-id");
			$($(this).attr("data-target")).modal("show");
		})
		.end().find(".search-image").on("click", function(e)
		{
