...
<div id="content" class="panel panel-default">
	<div class="panel-heading col-xs-12">
    	...
    	<div class="col-xs-6 text-center"><h5>Destination Relation</h5></div>
        <div class="col-xs-3 text-right"><a data-toggle="modal" data-target="#modal-add-relation" class="btn btn-danger" role="button">Add Destination Relation</a></div>
	</div>
...
<!-- START: Modal -->
	<?php echo $modal_add_relation; ?>
    <?php echo $modal_edit_relation; ?>
	<?php echo $modal_delete_relation; ?>
<!-- END: Modal -->
...
<script>
	var grid = $("#grid").bootgrid({
		...
		formatters: {
			...
			"target_id": function(column, row)
			{
				var target_id = JSON.parse(row.target_id);
				if(target_id !== '' && target_id !== 'undefined' && target_id !== null) {
					return "<a class=\"label label-pill search-target_id\" data-row-name=\"" + target_id.name + "\" style=\"background-color:"+target_id.type_color+";\">" + target_id.name + "</a>";
				}
			},
			...
			{
				return "<button type=\"button\" class=\"btn btn-default command-delete\" data-toggle=\"modal\" data-target=\"#modal-delete-relation\" data-row-id=\"" + row.id + "\"><span class=\"fa fa-fw fa-trash-o\"></span></button>";
	...
	}).on("loaded.rs.jquery.bootgrid", function()
	{
		...
		{
			document.getElementById("modal-get-relation-form-input-relation-id").value = $(this).data("row-id");
			getRelation(); <!-- the code is written in modal tpl -->
			$($(this).attr("data-target")).modal("show");
		})
		.end().find(".command-delete").on("click", function(e)
		{
			document.getElementById("modal-delete-relation-form-input-relation-id").value = $(this).data("row-id");
			document.getElementById("modal-delete-relation-form-text-relation-id").innerHTML = $(this).data("row-id");
			$($(this).attr("data-target")).modal("show");
		});
	});
