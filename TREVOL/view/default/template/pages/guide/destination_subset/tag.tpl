...
<div id="content" class="panel panel-default">
	<div class="panel-heading col-xs-12">
    	...
    	<div class="col-xs-8 text-center"><h5>Destination Tag</h5></div>
        <div class="col-xs-2 text-right"><a data-toggle="modal" data-target="#modal-add-tag" class="btn btn-danger" role="button">Add Destination Tag</a></div>
	</div>
...
<!-- START: Modal -->
	<?php echo $modal_add_tag; ?>
    <?php echo $modal_edit_tag; ?>
	<?php echo $modal_delete_tag; ?>
<!-- END: Modal -->
...
<script>
	var grid = $("#grid").bootgrid({
		...
		formatters: {
			...
			{
				return "<button type=\"button\" class=\"btn btn-default command-edit\" data-toggle=\"modal\" data-target=\"#modal-edit-tag\" data-row-id=\"" + row.id + "\"><span class=\"fa fa-fw fa-pencil\"></span></button> " + 
					"<button type=\"button\" class=\"btn btn-default command-delete\" data-toggle=\"modal\" data-target=\"#modal-delete-tag\" data-row-id=\"" + row.id + "\"><span class=\"fa fa-fw fa-trash-o\"></span></button>";
	...
	}).on("loaded.rs.jquery.bootgrid", function()
	{
		...
		{
			document.getElementById("modal-get-tag-form-input-relation-id").value = $(this).data("row-id");
			getTag(); <!-- the code is written in modal tpl -->
			$($(this).attr("data-target")).modal("show");
		})
		.end().find(".command-delete").on("click", function(e)
		{
			document.getElementById("modal-delete-tag-form-input-relation-id").value = $(this).data("row-id");
			document.getElementById("modal-delete-tag-form-text-relation-id").innerHTML = $(this).data("row-id");
			$($(this).attr("data-target")).modal("show");
		})
