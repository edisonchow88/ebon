...
<div id="content" class="panel panel-default">
	<div class="panel-heading col-xs-12">
    	...
    	<div class="col-xs-8 text-center"><h5>Poi Contact</h5></div>
        <div class="col-xs-2 text-right"><a data-toggle="modal" data-target="#modal-add-contact" class="btn btn-danger" role="button">Add Poi Contact</a></div>
	</div>
...
<!-- START: Modal -->
	<?php echo $modal_add_contact; ?>
    <?php echo $modal_edit_contact; ?>
	<?php echo $modal_delete_contact; ?>
<!-- END: Modal -->
...
<script>
	var grid = $("#grid").bootgrid({
		...
		formatters: {
			...
			{
				return "<button type=\"button\" class=\"btn btn-default command-edit\" data-toggle=\"modal\" data-target=\"#modal-edit-contact\" data-row-id=\"" + row.id + "\"><span class=\"fa fa-fw fa-pencil\"></span></button> " + 
					"<button type=\"button\" class=\"btn btn-default command-delete\" data-toggle=\"modal\" data-target=\"#modal-delete-contact\" data-row-id=\"" + row.id + "\"><span class=\"fa fa-fw fa-trash-o\"></span></button>";
	...
	}).on("loaded.rs.jquery.bootgrid", function()
	{
		...
		{
			document.getElementById("modal-get-contact-form-input-contact-id").value = $(this).data("row-id");
			getContact(); <!-- the code is written in modal tpl -->
			$($(this).attr("data-target")).modal("show");
		})
		.end().find(".command-delete").on("click", function(e)
		{
			document.getElementById("modal-delete-contact-form-input-contact-id").value = $(this).data("row-id");
			document.getElementById("modal-delete-contact-form-text-contact-id").innerHTML = $(this).data("row-id");
			$($(this).attr("data-target")).modal("show");
		});
	});
