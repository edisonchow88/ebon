...
<div id="content" class="panel panel-default">
	<div class="panel-heading col-xs-12">
    	...
    	<div class="col-xs-8 text-center"><h5>Trip Activity</h5></div>
        <div class="col-xs-2 text-right"><a data-toggle="modal" data-target="#modal-add-activity" class="btn btn-danger" role="button">Add Trip Activity</a></div>
	</div>
...
<!-- START: Modal -->
	<?php echo $modal_add_activity; ?>
    <?php echo $modal_edit_activity; ?>
	<?php echo $modal_delete_activity; ?>
<!-- END: Modal -->
...
<script>
	var grid = $("#grid").bootgrid({
		...
		formatters: {
			...
			{
				return "<button type=\"button\" class=\"btn btn-default command-edit\" data-toggle=\"modal\" data-target=\"#modal-edit-activity\" data-row-id=\"" + row.id + "\"><span class=\"fa fa-fw fa-pencil\"></span></button> " + 
					"<button type=\"button\" class=\"btn btn-default command-delete\" data-toggle=\"modal\" data-target=\"#modal-delete-activity\" data-row-id=\"" + row.id + "\"><span class=\"fa fa-fw fa-trash-o\"></span></button>";
	...
	}).on("loaded.rs.jquery.bootgrid", function()
	{
		...
		{
			document.getElementById("modal-form-get-activity-input-activity-id").value = $(this).data("row-id");
			getTripActivity(); <!-- the code is written in modal tpl -->
			$($(this).attr("data-target")).modal("show");
		})
		.end().find(".command-delete").on("click", function(e)
		{
			document.getElementById("modal-form-delete-activity-input-activity-id").value = $(this).data("row-id");
			document.getElementById("modal-form-delete-activity-text-activity-id").innerHTML = $(this).data("row-id");
			$($(this).attr("data-target")).modal("show");
		});
	});
