...
if (defined('ADMIN_PATH') && (isset($_GET['s']) || isset($_POST['s'])) && ($_GET['s'] == ADMIN_PATH || $_POST['s'] == ADMIN_PATH)) {
	define('IS_TREVOL', false); //ADDED by TREVOL, 2016/01/23
...
//START
} else if (defined('TREVOL_PATH') && (isset($_GET['s']) || isset($_POST['s'])) && ($_GET['s'] == TREVOL_PATH || $_POST['s'] == TREVOL_PATH)) {
	define('IS_TREVOL', true);
	define('IS_ADMIN', true);
	define('DIR_APP_SECTION', DIR_ROOT . '/TREVOL/');
	define('DIR_LANGUAGE', DIR_ROOT . '/TREVOL/language/');
	define('DIR_TEMPLATE', DIR_ROOT . '/TREVOL/view/');
	define('DIR_STOREFRONT', DIR_ROOT . '/storefront/');
	define('DIR_BACKUP', DIR_ROOT . '/TREVOL/system/backup/');
	define('DIR_DATA', DIR_ROOT . '/TREVOL/system/data/');
	//generate unique sessioin name. 
	//NOTE: This is a session name not to confuse with actual session id. Candidate to renaming 
	define('SESSION_ID', defined('UNIQUE_ID') ? 'AC_CP_'.strtoupper(substr(UNIQUE_ID, 0, 10)) : 'AC_CP_PHPSESSID');
//END
