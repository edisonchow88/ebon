...
final class ACustomer{
	...
	public function __construct($registry){
		...
		} else if(isset($this->request->cookie['customer'])){
	...
	/** ADDITIONAL FUNCTION BY TREVOL TEAM*/
	/**
	 * Add item to wisth list
	 * @param int $product_id
	 * @return null
	 */
	public function addToItineraryList($product_id){
		if(!has_value($product_id) || !is_numeric($product_id)){
			return null;
		}
		$idinerarylist = $this->getItineraryList();
		$idinerarylist[$product_id] = time();
		$this->saveItineraryList($idinerarylist);
		return null;
	}
	 ...
	 * @return null
	 */
	public function removeFromItineraryList($product_id){
		if(!has_value($product_id) || !is_numeric($product_id)){
			return null;
		}
		$idinerarylist = $this->getItineraryList();
		unset($idinerarylist[$product_id]);
		$this->saveItineraryList($idinerarylist);
		return null;
	}
	 ...
	 * @return null
	 */
	public function saveItineraryList($idinerarylist = array()){
		$customer_id = $this->customer_id;
		if(!$customer_id){
			$customer_id = $this->unauth_customer['customer_id'];
		}
		...
		$this->db->query("UPDATE " . $this->db->table("customers") . " SET itinerary_list = '" . $this->db->escape(serialize($idinerarylist)) . "', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "' WHERE customer_id = '" . (int)$customer_id . "'");
	}
	...
	public function getItineraryList(){
		...
		$customer_query = $this->db->query("SELECT itinerary_list FROM " . $this->db->table("customers") . " WHERE customer_id = '" . (int)$customer_id . "' AND status = '1'");
		if($customer_query->num_rows){
			//load customer saved cart
			if(($customer_query->row['itinerary_list']) && (is_string($customer_query->row['itinerary_list']))){
				return unserialize($customer_query->row['itinerary_list']);
			}
		}
		return array();
	}
